...
def find_dup_dict_result(comparisons):
    ...
    for comp in comparisons:
        ...
        for architecture in comp["tests_archs"]:
            ...
            command_to_execute = MAGIC_COMMAND_FIND_DUP_DICT.replace(
                "RELEASE_NAME", rel_name
            ).replace("ARCHITECTURE", architecture)
            comp[test_field].append(
                {"passed": get_status(command_to_execute), "arch": architecture}
            )
...
def find_one_class_versions_result(command_to_execute):
    ...
    file = out.strip()
    if (ret_code == 0) and (out != ""):
        out, err, ret_code = get_output_command(
            "grep '^ *FAILED:' %s/%s |wc -l" % (JENKINS_ARTIFACTS_DIR, file)
        )
...
def find_general_test_results(
    ...
    for comp in comparisons:
        rel_name = comp["compared_tags"].split("-->")[1]
        print("Looking for {0} results for {1}.".format(test_field, rel_name))
        command_to_execute = magic_command.replace("RELEASE_NAME", rel_name).replace(
...
def find_check_crab(comparisons, architecture):
    for comp in comparisons:
        ...
        comp["crab_tests"] = find_and_check_result(
            rel_name,
            architecture,
            CHECK_CRAB_PATH,
...
def find_check_headers(comparisons, architecture):
    for comp in comparisons:
        ...
        comp["check-headers"] = find_and_check_result(
            rel_name, architecture, CHECK_HEADERS_PATH, "cat {0} | wc -l"
        )
...
def find_one_static_check(release_name, architecture):
    ...
    if ret_code == 0:
        arch = out.split()[0]
        print("found", arch)
        filter1 = find_one_static_filter_check(
            release_name, arch, MAGIC_COMMAND_FIND_STATIC_CHECKS_FILTER1
        )
        return arch + ":" + filter1
    print("inprogress")
    return "inprogress"
...
def find_one_material_budget(release_name, architecture):
    ...
    """
    command_to_execute = MAGIC_COMMAND_FIND_MATERIL_BUDGET_CHECKS.replace(
    ...
    if ret_code == 0:
        arch = out.split()[0]
        print("found", arch)
        command_to_execute = MAGIC_COMMAND_FIND_MATERIL_BUDGET_COMPARISON_CHECKS.replace(
            "RELEASE_NAME", release_name
        ).replace("ARCHITECTURE", architecture)
        print("Running ", command_to_execute)
        out, err, ret_code = get_output_command(command_to_execute)
...
def get_production_archs(config_map):
    ...
    for release in config_map:
        if ("PROD_ARCH" in release) and (
            ("DISABLED" not in release) or ("IB_WEB_PAGE" in release)
        ):
...
def identify_release_groups(results):
    ...
    for rq in structure["all_prefixes"][::-1]:
        ...
        if (
            (rn in release_objs)
            and ("comparisons" in release_objs[rn])
            and (release_objs[rn]["comparisons"])
        ):
...
if __name__ == "__main__":
    MAGIC_COMMAND_CMSDIST_TAGS = (
        "pushd %s; git tag -l '*/*/ARCHITECTURE' | grep -E 'IB|ERR'; popd" % CMSDIST_REPO
    )
    CMSSDT_DIR = "/data/sdt"
    ...
    MAGIC_COMMAND_FIND_FIRST_MERGE_WITH_TAG = (
        "GIT_DIR="
        + CMSSW_REPO_LOCAL
        + ' git log --pretty=\'"%s", "tags->,%d"\' END_TAG | grep "\\"tags->," | head -n1'
    )
    MAGIC_COMMAD_FIND_RESULTS_RELVALS = (
        "find "
        + BUILD_LOG_DIR
    ...
    MAGIC_COMMAND_FIND_RESULTS_BUILD = (
        "find "
        + BUILD_LOG_DIR
        + '/ARCHITECTURE/www  -mindepth 5 -maxdepth 5 -path "*/new/logAnalysis.pkl"'
    )
    MAGIC_COMMAND_FIND_RESULTS_FWLITE = (
        "find "
        + BUILD_LOG_DIR
        + '/ARCHITECTURE/www  -mindepth 5 -maxdepth 5 -path "*/new_FWLITE/logAnalysis.pkl"'
    )
    MAGIC_COMMAND_FIND_RESULTS_PYTHON3 = (
        "find " + BUILD_LOG_DIR + "/ARCHITECTURE/www -mindepth 4 -maxdepth 4 -name python3.html"
    )
    MAGIC_COMMAND_FIND_INVALID_INCLUDES = (
        "find "
        + JENKINS_ARTIFACTS_DIR
        + '/invalid-includes -maxdepth 3 -mindepth 3 -path "*/ARCHITECTURE/summary.json" -type f'
    )
    MAGIC_COMMAND_FIND_STATIC_CHECKS = (
        "test -d "
        + JENKINS_ARTIFACTS_DIR
        + "/ib-static-analysis/RELEASE_NAME && ls "
        + JENKINS_ARTIFACTS_DIR
    ...
    MAGIC_COMMAND_FIND_STATIC_CHECKS_FILTER1 = (
        "test -s "
        + JENKINS_ARTIFACTS_DIR
        + "/ib-static-analysis/RELEASE_NAME/ARCHITECTURE/reports/modules2statics-filter1.txt && echo reports/modules2statics-filter1.txt"
    )
    MAGIC_COMMAND_FIND_MATERIL_BUDGET_CHECKS = (
        "test -d "
        + JENKINS_ARTIFACTS_DIR
        + "/material-budget/RELEASE_NAME && ls "
        + JENKINS_ARTIFACTS_DIR
        + "/material-budget/RELEASE_NAME/"
    )
    MAGIC_COMMAND_FIND_MATERIL_BUDGET_COMPARISON_CHECKS = (
        "TEST_FILE="
        + JENKINS_ARTIFACTS_DIR
        + "/material-budget/RELEASE_NAME/ARCHITECTURE/comparison/Images/MBDiff.txt && test -f $TEST_FILE && grep '0$' $TEST_FILE | wc -l"
    )
    ...
    MAGIC_COMMAND_FIND_DQM_TESTS = (
        "test -d " + JENKINS_ARTIFACTS_DIR + "/ib-dqm-tests/RELEASE_NAME"
    )
    ...
    CHECK_HEADERS_PATH = (
        JENKINS_ARTIFACTS_DIR + "/check_headers/RELEASE_NAME/ARCHITECTURE/headers_with_error.log"
    )
    CHECK_UBSANLOG_PATH = JENKINS_ARTIFACTS_DIR + "/ubsan_logs/CMSSW_*/ubsan_runtime_errors.log"
    MAGIC_COMMAND_FIND_FLAWFINDER = (
        "test -d " + JENKINS_ARTIFACTS_DIR + "/flawfinder/RELEASE_NAME/ARCHITECTURE"
    )
    MAGIC_COMMAND_FIND_DUP_DICT = (
        "grep -v '^Searching for '  "
        + BUILD_LOG_DIR
        + "/ARCHITECTURE/www/*/*/RELEASE_NAME/testLogs/dup*.log"
    ...
    for comp in REQUESTED_COMPARISONS:
        ...
        if re.search(SP_REL_REGEX, release_queue):
            ...
            release_queue = re.match(r"^((?:[^_]*_){%d}[^_]*)_(.*)" % (4), release_queue).groups()[
                0
            ]
        else:
            release_queue = re.match(r"^((?:[^_]*_){%d}[^_]*)_(.*)" % (3), release_queue).groups()[
                0
            ]
        ...
        tags = execute_magic_command_tags(
            start_tag,
            end_tag,
            release_queue,
            release_branch,
        ...
        originalTags = tags
        tags = [
            x for x in tags if x in installedReleases
        ]  # NOTE: comment out on local development
        tags.append(release_branch)
        print("I got these tags: ")
        print(tags)

        print("Getting merged pull requests between tags...")
        release_queue_results["comparisons"] = compare_tags(
            release_branch, tags, CMSSW_REPO_LOCAL, CMSSW_REPO
        )
        print("Done")
